...
const MOCK_STATIC_PROCESS_CHAT_FN = jest.fn<
...
describe('ChatController', () => {
  ...
  describe('handleChatMessage', () => {
    ...
    it('should handle operational service errors with specific status and message', async () => {
      const operationalError = new Error("Location not supported by service") as GeminiChatServiceError;
      operationalError.isOperational = true;
      operationalError.statusCode = 403;
      operationalError.userMessage = "Service not available in your region due to location.";
    ...
    it('should return 400 if newUserMessage is missing (validation)', async () => {
      ...
      expect(mockResponse.json).toHaveBeenCalledWith(expect.objectContaining({ error: 'Missing required fields' }));
    });
