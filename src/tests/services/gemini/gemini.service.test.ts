...
const mockReadFileSync = jest
  .fn()
  .mockReturnValue("Mocked prompt injection detection prompt text from test.");
jest.mock("fs", () => ({
...
type MockSDKCandidate = {
  content: MockSDKContent;
  finishReason: string;
  safetyRatings?: any[];
};
...
} from "../../../src/types/gemini/gemini.js";
...
describe("GeminiService Instance Methods", () => {
  beforeEach(() => {
    ...
    mockSendMessage.mockResolvedValue({
      ...
      candidates: [
        {
          content: {
            parts: [{ text: "Mocked AI response from generateText" }],
          },
          finishReason: "STOP",
        },
      ],
    ...
    mockGenerateContent.mockResolvedValue({
      text: JSON.stringify({
        is_malicious: false,
        reason: "Input appears safe from evaluation",
      }),
      candidates: [
        {
          content: {
  ...
  describe("generateText", () => {
    ...
    it("should successfully generate text with valid input", async () => {
      ...
      expect(mockSendMessage).toHaveBeenCalledWith({
        message: validPayload.newUserMessage,
      });
    });
    ...
    it("should handle safety blocks and throw a GeminiChatServiceError", async () => {
      ...
      const payload = {
        ...validPayload,
        newUserMessage: "Content that gets blocked",
      };
      await expect(geminiService.generateText(payload)).rejects.toThrow(
        expect.objectContaining<Partial<GeminiChatServiceError>>({
    ...
    it("should handle API errors (e.g., network issue) and throw GeminiChatServiceError", async () => {
      ...
      await expect(geminiService.generateText(validPayload)).rejects.toThrow(
        expect.objectContaining<Partial<GeminiChatServiceError>>({
  ...
  describe("evaluatePromptSafety", () => {
    ...
    it("should evaluate prompt safety successfully and return not malicious", async () => {
      const result = await geminiService.evaluatePromptSafety(
        systemInput,
        userInput,
      );
        ...
        expect.objectContaining({
          ...
          contents:
            ...
            expect.stringContaining(
              "Mocked prompt injection detection prompt text from test.",
            ),
          config: expect.objectContaining({
    ...
    it("should handle malicious content detection by the evaluation model", async () => {
      const maliciousEvalResponse = {
        is_malicious: true,
        reason: "AI detected prompt injection attempt.",
      };
      mockGenerateContent.mockResolvedValueOnce({
        ...
        candidates: [
          {
            content: {
              parts: [{ text: JSON.stringify(maliciousEvalResponse) }],
            },
            finishReason: "STOP",
          },
        ],
    ...
    it("should handle safety blocks (promptFeedback) during evaluation", async () => {
      ...
      const result = await geminiService.evaluatePromptSafety(
        systemInput,
        "Content blocked during eval",
      );
      expect(result.isMalicious).toBe(false);
      expect(result.reason).toBe(
        "Evaluation failed: Input prompt blocked due to EVAL_SAFETY_BLOCK.",
      );
    });
    ...
    it("should handle candidate finishReason SAFETY during evaluation", async () => {
      ...
      const result = await geminiService.evaluatePromptSafety(
        systemInput,
        "Dangerous eval input",
      );
      expect(result.isMalicious).toBe(false);
      expect(result.reason).toMatch(
        /Evaluation model stopped generation due to SAFETY. Safety ratings: \[HARM_CATEGORY_DANGEROUS_CONTENT: HIGH\]/,
      );
    });
    ...
    it("should handle unparsable JSON from evaluation model", async () => {
      ...
      expect(result.isMalicious).toBe(false);
      expect(result.reason).toBe(
        "Evaluation failed: Could not parse model's JSON response.",
      );
    });
