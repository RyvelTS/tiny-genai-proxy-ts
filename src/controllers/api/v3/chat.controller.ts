...
import AiModelService, {
  AiService,
  Credentials,
} from "../../../services/ai-model.service.js";
...
class ChatController {
  ...
  public static async handleChatMessage(
    ...
    try {
      ...
      const azureEndpoint = req.headers["x-azure-endpoint"] as
        | string
        | undefined;
      ...
      if (!service || !Object.values(AiService).includes(service)) {
        res
          .status(400)
          .json({ error: 'A valid "service" query parameter is required.' });
        return;
      }
      if (!model) {
        res
          .status(400)
          .json({
            error: 'The "model" field is missing from the request body.',
          });
        return;
      }
      if (!Array.isArray(messages) || messages.length === 0) {
        res
          .status(400)
          .json({ error: 'The "messages" field must be a non-empty array.' });
        return;
      }
      if (!apiKey) {
        res
          .status(401)
          .json({ error: 'The "X-API-Key" header is missing or empty.' });
        return;
      }

      const credentials: Credentials = { apiKey, azureEndpoint };
      logger.info(
        `Request received for chat with service: ${service}, model: ${model}`,
      );
      ...
      logger.error(
        `Error in handleChatMessage for service "${req.body.service}":`,
        error,
      );
  ...
  public static async getAvailableModels(
    ...
    try {
      ...
      logger.error(
        `Error in getAvailableModels for service "${req.query.service}":`,
        error,
      );
      res.status(500).json({
        error:
