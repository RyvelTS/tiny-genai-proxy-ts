...
class GeminiChatService {
  public async processChat(payload: GeminiChatRequestPayload) {
    ...
    const evaluationResult = await GeminiService.evaluatePromptSafety(
      systemPrompt,
      newUserMessage,
    );
    ...
    if (evaluationResult.isMalicious) {
      ...
      if (payload.conversationHistory) {
        payload.conversationHistory.push({
          role: "model",
          parts: [evalResult],
        });
        userMessageForModel =
          "Why can't you help me? And how can you assist me today?";
      } else {
    ...
    const aiResponseText = await GeminiService.generateText({
      ...payload,
      newUserMessage: userMessageForModel,
    });
