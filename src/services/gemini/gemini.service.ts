...
let PROMPT_INJECTION_DETECTION_PROMPT =
  "Error: Could not load prompt injection detection prompt.";
try {
  const promptFilePath = path.join(
    process.cwd(),
    "src",
    "prompts",
    "detect-prompt-injection.txt",
  );
  PROMPT_INJECTION_DETECTION_PROMPT = fs.readFileSync(promptFilePath, "utf8");
} catch (err) {
  logger.error(
    "Failed to load prompt injection detection prompt from file:",
    err,
  );
}
...
class GeminiService {
  ...
  constructor() {
    ...
    this.evaluationModelName =
      process.env.GEMINI_EVALUATION_MODEL || "gemini-2.0-flash";
  }
  ...
  private parseGeminiApiError(error: unknown): {
    ...
    if (lowerMsg.includes("quota exceeded")) {
      return {
        userMessage:
          "Sorry, this service is currently experiencing heavy traffic or has reached its usage limit. Please try again later.",
        statusCode: 429,
        isQuotaError: true,
        details: message,
      };
  ...
  public async evaluatePromptSafety(
    systemInput: string,
    userInput: string,
    ...
    try {
          ...
          responseSchema: {
            ...
            properties: {
              is_malicious: {
                type: "boolean",
                description:
                  "True if the user input is considered malicious (e.g., prompt injection), false otherwise.",
              },
              reason: {
                type: "string",
      ...
      if (response.promptFeedback?.blockReason) {
        ...
        logger.warn(
          `Prompt was blocked by safety filters during evaluation: ${blockReason}.`,
        );
        return {
          isMalicious: false,
          reason: `Evaluation failed: Input prompt blocked due to ${blockReason}.`,
        };
      }
      if (!response.candidates?.length) {
        logger.warn("No candidates returned from evaluation model.", {
          promptFeedback: response.promptFeedback,
        });
        return {
          isMalicious: false,
          reason: "Evaluation failed: No response generated by the model.",
        };
      }
      ...
      if (!rawText) {
        logger.warn("No text part found in the candidate response.", {
          candidate,
        });
        return {
          isMalicious: false,
          reason: "Evaluation failed: Model returned an empty response part.",
        };
      }
      ...
      try {
        ...
        try {
          ...
          logger.error(
            "Failed to parse JSON from prompt evaluation response.",
            { rawText },
          );
          return {
            isMalicious: false,
            reason: "Evaluation failed: Could not parse model's JSON response.",
          };
        }
      }
      if (
        typeof evaluation?.is_malicious !== "boolean" ||
        typeof evaluation?.reason !== "string"
      ) {
        logger.error(
          "Parsed evaluation JSON is missing required fields or has incorrect types:",
          { evaluation },
        );
        return {
          isMalicious: false,
          reason:
            "Evaluation failed: Model response did not match expected schema structure (missing/invalid fields).",
        };
      }
      ...
      return {
        ...
        reason:
          evaluation.reason ||
          (evaluation.is_malicious
            ? "No specific reason provided by evaluator."
            : "Input classified as not malicious."),
      };
    } catch (error) {
      ...
      if (parsed.isLocationError) {
        return {
          isMalicious: false,
          reason: parsed.userMessage || "Service not available in your region.",
        };
      }
      if (parsed.isQuotaError) {
        return {
          isMalicious: false,
          reason: parsed.userMessage || "Service quota exceeded.",
        };
      }
      if (parsed.isApiKeyError) {
        return {
          isMalicious: false,
          reason: parsed.userMessage || "API key error.",
        };
      }

      return {
        isMalicious: false,
        reason: parsed.details || "Prompt evaluation service error.",
      };
    }
  }
  ...
  public async generateText(
    ...
    try {
      ...
      const chatHistory: Content[] = (conversationHistory || []).map(
        (msg): Content => {
          let textContent = "";
          if (msg.parts?.length) {
      ...
      if (response.promptFeedback?.blockReason) {
        const serviceError: GeminiChatServiceError = Object.assign(
          new Error(`Content blocked: ${response.promptFeedback.blockReason}`),
          {
            isOperational: true,
            statusCode: 400,
      ...
      if (!response.candidates?.length || !response.candidates[0].content) {
        const serviceError: GeminiChatServiceError = Object.assign(
          new Error("No content generated by AI model."),
          {
            isOperational: true,
            statusCode: 500,
            userMessage:
      ...
      const parsed = this.parseGeminiApiError(error);
      const serviceError: GeminiChatServiceError = Object.assign(
        new Error("Failed to generate chat response."),
        {
          isOperational: true,
        },
      );
      if (parsed.isLocationError) {
        ...
        serviceError.userMessage = parsed.userMessage;
        serviceError.message =
          parsed.details || parsed.userMessage || "Service quota exceeded.";
      } else if (parsed.isApiKeyError) {
        serviceError.statusCode = 500;
        serviceError.userMessage = parsed.userMessage;
        serviceError.message =
          parsed.details || parsed.userMessage || "API key error.";
  ...
  public async generateResponse(
    ...
    try {
      ...
      if (finalConfig.responseSchema && typeof response.text === "string") {
        try {
          parsedResponse = Object.assign({}, response, {
            text: JSON.parse(response.text),
          }) as GenerateContentResponse;
        } catch (e) {
          logger.warn(
            "Failed to parse response.text as JSON despite responseSchema being present.",
            { text: response.text },
          );
        }
      }
      ...
      if (parsedResponse.promptFeedback?.blockReason) {
        const serviceError: GeminiChatServiceError = Object.assign(
          new Error(
            `Content blocked: ${parsedResponse.promptFeedback.blockReason}`,
          ),
          {
            isOperational: true,
            statusCode: 400,
      ...
      if (
        !parsedResponse.candidates?.length ||
  ...
  public async createChatCompletionWithKey(
    ...
    try {
      ...
      logger.error(
        `Error in GeminiService.createChatCompletionWithKey for model ${model}:`,
        error,
      );
      ...
      if (
        parsed.isLocationError ||
        parsed.isQuotaError ||
        parsed.isApiKeyError
      ) {
        serviceError.statusCode = parsed.statusCode;
        serviceError.userMessage = parsed.userMessage;
        serviceError.message =
          parsed.details || parsed.userMessage || "A known API error occurred.";
      } else if ((error as GeminiChatServiceError).isOperational) {
        throw error;
      } else {
        serviceError.statusCode = 500;
